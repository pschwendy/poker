#############################################
# State file specific to Fold Hold'em Poker #
#############################################
...
class State:
    ...
    def __init__(self, n_players=6) -> None:
        ...
        self.table = []
        self.mini_states = [MiniState(self.table, self.active, self.total_players)]
    ...
    def finish_round(self):
        ...
        self.mini_states.append(MiniState(self.table, self.active, self.total_players))
    ...
    def is_terminal(self) -> bool:
        return (self.round == Round.FLOP and self.end_round()) or self.one_player_left() or len(self.mini_states) > 2
    ...
    def to_dict(self, i):
        ...
        return {
            "h_action": torch.stack([torch.Tensor(mini_state.encode_action()).to(device) for mini_state in self.mini_states] + [torch.zeros(7, 1).to(device) for _ in range(2-len(self.mini_states))]).view(-1),
            "cards": [Card(c).encode() for c in self.table] + [-1 for _ in range(3 - len(self.table))],
        }
